...
SWIFT_CLASS("_TtC8RadarSDK23RadarEventNotifications")
@interface RadarEventNotifications : NSObject
/// Show local notifications for incoming events (geofence, beacon, trip).
/// Called by RadarDelegateHolder when events arrive.
+ (void)showNotificationsFor:(NSArray<RadarEvent *> * _Nonnull)events;
/// Builds notification content from campaign metadata. Returns nil if not a campaign
/// or missing required fields.
+ (UNMutableNotificationContent * _Nullable)extractCampaignContentFrom:(NSDictionary * _Nullable)metadata identifier:(NSString * _Nullable)identifier SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
SWIFT_CLASS("_TtC8RadarSDK26RadarNotificationSwizzling")
@interface RadarNotificationSwizzling : NSObject
+ (void)swizzleNotificationCenterDelegate;
+ (void)swizzleApplicationDelegate;
+ (void)openURLFromNotification:(UNNotification * _Nonnull)notification;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

@class UNNotificationRequest;
SWIFT_CLASS("_TtC8RadarSDK22RadarNotificationUtils")
@interface RadarNotificationUtils : NSObject
+ (void)checkNotificationPermissionsWithCompletionHandler:(void (^ _Nullable)(BOOL))completionHandler;
/// Returns delivered notifications (registered minus still-pending).
/// Called from RadarAPIClient.m before each track request.
+ (void)getNotificationDiffWithCompletionHandler:(void (^ _Nonnull)(NSArray<NSDictionary<NSString *, id> *> * _Nonnull, NSArray * _Nonnull))completionHandler;
